...
case class Generic(
  ...
  override val params:     Option[Map[String, Any]]) extends Expression
...
case class Alias(
  alias: String,
  child: Expression) extends Expression
...
case class Binary(
  symbol:                  String,
  override val dataTypeId: UUID,
  override val children:   Seq[Expression]) extends Expression
  with TypedExpression

case class AttrRef(refId: UUID) extends Expression with LeafExpression with RefLikeExpression

case class Literal(
  value:                   Any  = null, // Salat requires nullable non-Option properties to have default values.
  override val dataTypeId: UUID) extends Expression
  with LeafExpression
  with TypedExpression
