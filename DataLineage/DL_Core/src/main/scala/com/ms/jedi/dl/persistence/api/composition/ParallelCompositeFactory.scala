...
class ParallelCompositeFactory(configuration: Configuration) extends PersistenceFactory(configuration) {
  ...
  private val factories =
    configuration
      ...
      .map(className => {
        println(s"Instantiating underlying factory: $className")
  ...
  /**
    * The method creates a parallel composite writer to various persistence layers for the [[com.ms.jedi.dl.model.DataLineage DataLineage]] entity.
    *
    * @return A parallel composite writer to various persistence layers for the [[com.ms.jedi.dl.model.DataLineage DataLineage]] entity
    */
  override def createDataLineageWriter: DataLineageWriter =
    new ParallelCompositeDataLineageWriter(factories.map(factory => {
      println("${factory.getClass.getName}: create writer")
      factory.createDataLineageWriter
    }))

  /**
    * The method creates a reader from the persistence layer for the [[com.ms.jedi.dl.model.DataLineage DataLineage]] entity.
    *
    * @return An optional reader from the persistence layer for the [[com.ms.jedi.dl.model.DataLineage DataLineage]] entity
    */
