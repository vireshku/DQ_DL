...
object MongoPersistenceFactory {
  val mongoDbUrlKey = "vk.dl.mongodb.url"
  val mongoDbNameKey = "vk.dl.mongodb.name"
}
...
class MongoPersistenceFactory(configuration: Configuration) extends PersistenceFactory(configuration) {
  ...
  /**
    * The method creates a persistence writer for the [[com.ms.jedi.dl.model.DataLineage DataLineage]] entity.
    *
    * @return A persistence writer for the [[com.ms.jedi.dl.model DataLineage]] entity
    */
