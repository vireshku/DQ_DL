...
package com.ms.jedi.dl.model.op

import java.util.UUID

import salat.annotations.{ Persist, Salat }
import com.ms.jedi.dl.model.expr.Expression
import com.ms.jedi.dl.model.MetaDataSource
import com.ms.jedi.dl.model.TypedMetaDataSource
...
case class Join(
  ...
  joinType:  String) extends Operation with ExpressionAware {
...
case class Filter(
  mainProps: OperationProps,
  condition: Expression) extends Operation with ExpressionAware {
...
case class Aggregate(
  ...
  aggregations: Map[String, Expression]) extends Operation with ExpressionAware {
...
case class Sort(
  mainProps: OperationProps,
  orders:    Seq[SortOrder]) extends Operation
...
case class Projection(
  mainProps:       OperationProps,
  transformations: Seq[Expression]) extends Operation with ExpressionAware {
...
case class Alias(
  mainProps: OperationProps,
  alias:     String) extends Operation
...
case class Write(
  ...
  readMetrics:     Map[String, Long]) extends Operation
...
case class Read(
  mainProps:  OperationProps,
  sourceType: String,
  sources:    Seq[MetaDataSource]) extends Operation {
...
case class Composite(
  ...
  timestamp:   Long,
  appId:       String,
  appName:     String) extends Operation {
